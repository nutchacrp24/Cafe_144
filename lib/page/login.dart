...
import 'package:cafe_144/page/show_product.dart';
...
class LoginScreen extends StatelessWidget {
  ...
  Future<String?> _authUser(LoginData data) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://642021144.pungpingcoding.online/api/login'),
        body: {
          'email': data.name,
  ...
  Future<String?> _signupUser(SignupData data) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://642021144.pungpingcoding.online/api/register'),
  ...
  Widget build(BuildContext context) {
    return FlutterLogin(
      // title: 'NUTTY CAFE',
      theme: LoginTheme(
          primaryColor: Color.fromARGB(255, 255, 209, 93),
